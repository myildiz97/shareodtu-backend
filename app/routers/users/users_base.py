...
from models.user_model.user_model import User, CreateUser, UpdateUser, RegisterVendor, UserType
from services.users.user_services import (
    ...
    get_user_type_by_email as get_user_type_by_email_service,
        save_image as save_image_service
    )
...
async def register_vendor(full_name: str = Form(...),
    email: EmailStr = Form(...),
    password: str = Form(...),
    vendor_address: Optional[str] = Form(None),
    facility_name: Optional[str] = Form(None),
    vendor_phone: Optional[str] = Form(None),
    vendor_identity_no: Optional[str] = Form(None),
    image: UploadFile = File(...)
    ):
    image_data = await save_image_service(image)
    form_data_dict = {
        "full_name": full_name,
        "email": email,
        "password": password,
    ...
    form_data = RegisterVendor(**form_data_dict)
...
async def get_user_image(user_id: str, current_user: User = Depends(get_current_active_user)):
